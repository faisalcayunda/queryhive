...
/// A ZIP holding stored entries, which is all an `.xlsx` needs.
///
/// Uncompressed because the parts are small, the sheet is written once, and a deflate
/// implementation would be a dependency and a source of bugs for no gain the user can
/// see. The one entry that matters is the sheet, and it is streamed: its CRC and size
/// are not known when its local header has to be written, so the header declares a
/// **data descriptor** (flag bit 3) and the values follow the data.
struct Zip {
    out: BufWriter<File>,
    offset: u64,
...
impl Zip {
    ...
    fn write_data(&mut self, bytes: &[u8]) -> Result<(), ExportError> {
    ...
    fn add_stored(&mut self, name: &str, bytes: &[u8]) -> Result<(), ExportError> {
        let crc = Crc32::of(bytes);
        let offset = self.offset;
        let header = local_header(name, crc, bytes.len() as u64, false)?;
    ...
    fn begin_streamed(&mut self, name: &str) -> Result<usize, ExportError> {
        let offset = self.offset;
        ...
        let header = local_header(name, 0, 0, true)?;
        self.write_data(&header)?;
        self.entries.push(CentralEntry {
            name: name.to_owned(),
            crc: 0,
...
fn local_header(name: &str, crc: u32, size: u64, streamed: bool) -> Result<Vec<u8>, ExportError> {
...
impl Crc32 {
    ...
    /// Bitwise rather than table-driven: the parts here are a few kilobytes, and a
    /// 256-entry table would be a constant to keep correct for no measurable gain.
